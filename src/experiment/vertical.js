...
const inst = experiment('vertical',{
  init
  , exit
  , handleAnimate
  , handleResize
})
const zuper = inst.zuper

// private variables
let w,h,hMax,target
let canvas,context
let patternCanvas,patternContext
let img,imgTemp,imgPreview,imgPreviewUri,feedback
const margin = 8
let millisOffset = 0
const millisScale = 0.01
let switchMillLst = 0
let switchMillCnt = 0
...
function handleAnimate(deltaT,millis) {
  if (img) {
    ...
    let iMaxH = hMax - margin,iMil = millisScale * millis << 0,iMill = iMil % iMaxH,
      iOff = iMaxH - Math.abs(2 * iMill - iMaxH),sx = 1,sy = 0
